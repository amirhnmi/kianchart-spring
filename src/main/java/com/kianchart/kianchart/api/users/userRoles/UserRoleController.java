...
public class UserRoleController {
    ...
    public CustomResponseEntity<List<UserRoleDTO.Response>> getAllUserRole(
            @RequestParam(defaultValue = "asc") SortDirection sort,
            @RequestParam(defaultValue = "0") int skip,
            @RequestParam(defaultValue = "20") int limit,
            @RequestParam(defaultValue = "false") boolean total
    ) {
        List<UserRoleDTO.Response> userRoles = userRoleService.getAllUserRole(sort, skip, limit);
    ...
    public CustomResponseEntity<UserRoleDTO.Response> createUserRole(
            @Valid @RequestBody UserRoleDTO.CreateUserRoleRequest createRequest
    ) {
        UserRoleDTO.Response userRole = userRoleService.createUserRole(createRequest);
